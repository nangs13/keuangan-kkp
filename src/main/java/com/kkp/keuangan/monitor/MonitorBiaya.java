...
import com.kkp.keuangan.monitor.component.ButtonEditorBiaya;
import com.kkp.keuangan.monitor.component.ButtonRenderer;
import com.kkp.keuangan.swing.ScrollBar;

public class MonitorBiaya extends JPanel {
    ...
    private void initComponents() {
        ...
        JLabel lblTitle = new JLabel("Daftar Biaya");
    ...
    private void loadAllData() {
        ...
        BiayaDAO dao = new BiayaDAO();
        CoaDAO coaDAO = new CoaDAO();

        List<ModelBiaya> list = dao.getSummaryAll();
        ...
        for (ModelBiaya b : list) {

            String tglUI = "";
            try {
                LocalDate ld = LocalDate.parse(b.getTanggal().toString().split(" ")[0], dbFmt); // ambil date saja
                tglUI = ld.format(uiFmt);
            } catch (Exception e) {
                tglUI = b.getTanggal().toString();
            }

            ModelCoa sumberCoa = coaDAO.findByCode(b.getSumberCode());
            String sumberName = (sumberCoa != null) ? sumberCoa.getNama() : "Unknown (" + b.getSumberCode() + ")";

            allData.add(new Object[]{
                    b.getCode(),
                    tglUI,
                    sumberName,
                    b.getJumlah(), // total sum
                    b.getKeterangan().split(" | Tujuan: ")[0],
            });
        }
    ...
    private void refreshTable() {
        ...
        List<ModelBiaya> pageData = dao.findPageSummary(rowsPerPage, offset); 
    
        DateTimeFormatter dbFmt = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        DateTimeFormatter uiFmt = DateTimeFormatter.ofPattern("dd MMM yyyy", Locale.ENGLISH);
    
        for (ModelBiaya b : pageData) {
    
            // format tanggal
            String tglUI;
            try {
                LocalDate ld = LocalDate.parse(b.getTanggal().toString().split(" ")[0], dbFmt);
                tglUI = ld.format(uiFmt);
            } catch (Exception e) {
                tglUI = b.getTanggal().toString();
            }
            ...
            // ambil nama sumber
            ModelCoa sumberCoa = coaDAO.findByCode(b.getSumberCode());
            String sumberName = (sumberCoa != null) ? sumberCoa.getNama() : b.getSumberCode();
    ...
    private void searchData() {
        ...
        List<ModelBiaya> list = dao.searchSummary(rowsPerPage, 0, keyword);
        model.setRowCount(0);
